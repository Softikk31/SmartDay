...
fun TaskFormDialog(state: TaskFormState, taskViewModel: TaskViewModel, locale: Locale) {
    ...
    val imageVector = if (showTimeDialog.value) ImageVector.vectorResource(R.drawable.ic_keyboard)
    ...
    val counterTextFieldStateRepetitionTypeCustom = key(dialogMode) {
        rememberTextFieldState(
            initialText = state.repetition.counter
        )
    }
    ...
    LaunchedEffect(counterTextFieldStateRepetitionTypeCustom.text) {
        ...
        if (current.isEmpty()) {
            ...
            counterTextFieldStateRepetitionTypeCustom.edit {
                replace(0, current.length, 1.toString())
            }
        }
    }
    ...
    when (dialogMode) {
        ...
        TaskFormDialogMode.TIME_DIALOG_MODE -> {
            CustomDialogTimePicker(
                ...
                icon = {
                    Icon(
                        modifier = Modifier
                            .size(if (imageVector == ImageVector.vectorResource(R.drawable.ic_clock)) 22.5.dp else 25.dp)
                            .clickable(
        ...
        TaskFormDialogMode.SELECT_REPETITION_DIALOG_MODE -> {
            CustomDialog(
                title = stringResource(R.string.task_create_button_title_repeat),
                titleStyle = MaterialTheme.typography.bodyLarge.copy(
                    fontWeight = FontWeight.Medium
                ),
                titleColor = MaterialTheme.colorScheme.onSurface,
                ...
                Column(
                    ...
                    ButtonRadio(
                        ...
                        selectRepetition.value = TaskRepetitionModel(
                            value = null, type = TaskTypeRepetition.OnSystemTypeRepetition(
                                TaskTypeRepetitionSystem.NULL
                            )
                        )
                    }
                    ...
                    ButtonRadio(
                        ...
                        title = stringResource(
                            R.string.repeat_every_week_on, state.date?.dayOfWeek?.getDisplayName(
                    ...
                    ButtonRadio(
                        ...
                        selectRepetition.value = TaskRepetitionModel(
                            value = YearDateRepetition(
                                ValueDateRepetition(
                                    month = state.date?.month?.value ?: LocalDate.now().month.value,
                        ...
                        IconButton(
                            enabled = if (counterTextFieldStateRepetitionTypeCustom.text.toString()
                                ...
                                try {
                                    ...
                                    Toast.makeText(
                                        context,
                                        e.message ?: "Error: NumberFormatException",
                                        Toast.LENGTH_LONG
                                    )
                                }
                            }) {
                    ...
                    Column(
                        ...
                        DropdownMenu(
                            ...
                            DropdownMenuItem(text = {
                                ...
                                selectTypeRepetitionTypeCustom.value = TaskTypeRepetitionCustom.DAY
                                expanded = false
                            })
                            DropdownMenuItem(text = {
                                Text(
                                ...
                                selectTypeRepetitionTypeCustom.value = TaskTypeRepetitionCustom.WEEK
                                expanded = false
                            })
                            DropdownMenuItem(text = {
                                Text(
                            ...
                            DropdownMenuItem(text = {
                                ...
                                selectTypeRepetitionTypeCustom.value = TaskTypeRepetitionCustom.YEAR
                                expanded = false
                            })
                        }
                    }
